<template>
  <div style="position: relative;" class="at-container" @focusout="inputBlur">
    <div ref="atInputRef" :class="_class" :style="{
      '--at-input-height': height
    }" :contenteditable="!disabled" spellcheck="false" @input="inputChange" @click="inputClick"
      @keydown="inputKeydown">
      <template v-html="inputValue"></template>
    </div>
    <div ref="popoverRef" v-if="popoverIsShow" class="at-popover" key="popover" tabindex="0" :style="{
      ...
      <ul v-else class="at-popover-list">
        <li v-for="(item, index) in options" class="at-popover-item"
          :class="{ 'active': currentFocusOptionIndex === index }" ref="popoverItemRef" :key="item.label"
          @mousedown="beforeSelect" @click="onSelect(item)">
          {{ item.label }}
        </li>
...
const emit = defineEmits<{
  (event: 'atChange', list: { key: string, label: string }[]): void;
}>();
...
function inputBlur(e: FocusEvent) {
  if (e.relatedTarget && (e.relatedTarget as HTMLElement).classList.contains('at-popover')) return;
  popoverIsShow.value = false;
  options.value = [];
}
...
function inputKeydown(e: KeyboardEvent) {
  switch (e.key) {
    ...
    case 'Backspace':
      ...
      if ((range.commonAncestorContainer as HTMLElement)?.classList?.contains('at-input')) {
        setTimeout(() => {
          emit('atChange', getAtList());
        }, 0)
      }
...
// 获取@数组
function getAtList() {
  return Array.prototype.map.call((atInputRef.value?.querySelectorAll('.at-span') as NodeList), (v: HTMLElement) => {
    return {
